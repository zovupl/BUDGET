...
function localParse(text, cats) {
  ...
  if (isIncome) {
    ...
    if (catId === "other_out") {
      // совпадение с названием любой (в том числе созданной вручную) категории
...
export default function FamilyBudget() {
  const [data, setData] = useState({ ...DEFAULT_DATA, transactions: [], debtPayments: [] });
  ...
  // окно ручного ввода траты/дохода
  const [sheet, setSheet] = useState(null); // { type, amount, category, note, date }
  ...
  // ── запись данных: каждая операция по своему пути, поэтому записи Мити и Марины не затирают друг друга ──
  const addTx = (type, personId, category, amount, note, dateIso) => {
    const id = newId();
    const tx = { id, type, personId, category, amount, note: note || "", date: dateIso || new Date().toISOString() };
  ...
  // ── ответы на вопросы: считаются на месте, интернет не нужен ──
  ...
  // ── ручной ввод: кнопки «Трата» / «Доход» ──
  const todayInput = () => {
    const d = new Date();
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
  };
  const openSheet = (type) =>
    setSheet({ type, amount: "", category: data.categories[type][0]?.id, note: "", date: todayInput() });
  ...
  const saveSheet = () => {
    ...
    // к выбранной дате подставляем текущее время — порядок записей за день сохраняется
    const [y, m, d] = sheet.date.split("-").map(Number);
    const t = new Date();
    const iso = new Date(y, m - 1, d, t.getHours(), t.getMinutes(), t.getSeconds()).toISOString();
    addTx(sheet.type, activePerson, sheet.category, amount, sheet.note.trim(), iso);
    const c = catById(sheet.category);
    setSheet(null);
    showFlash(`${sheet.type === "income" ? "📥" : "📤"} ${c.emoji} ${fmt(amount)} zł записано`);
  };
  ...
  // ── фото профиля: сжимаем до 120px, чтобы легко влезло в базу ──
  const uploadAvatar = (personId, file) => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        savePeople(data.people.map((p) => (p.id === personId ? { ...p, avatar: dataUrl } : p)));
            ...
            <div className="w-full max-w-md bg-white rounded-t-3xl p-5 pb-8 max-h-[92vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-black text-xl text-slate-800">
                  {sheet.type === "expense" ? "📤 Новая трата" : "📥 Новый доход"}
                </h2>
              ...
              <div className="grid grid-cols-4 gap-2 mb-4">
                {data.categories[sheet.type].map((c) => {
                  ...
                  return (
                    <button key={c.id} onClick={() => setSheet({ ...sheet, category: c.id })}
