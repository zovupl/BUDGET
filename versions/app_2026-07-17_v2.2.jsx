...
// ── донат-диаграмма ──
function Donut({ items, total, centerLabel, size = 170 }) {
  const R = 70, C = 2 * Math.PI * R;
  let acc = 0;
  return (
    <svg width={size} height={size} viewBox="0 0 170 170" className="shrink-0 mx-auto block">
...
export default function FamilyBudget() {
  ...
  const [spendingOpen, setSpendingOpen] = useState(false); // «Куда ушли деньги»: свёрнуто/развёрнуто
  const [openCat, setOpenCat] = useState(null);            // раскрытая категория в «Куда ушли деньги» → список транзакций
  ...
  // ── сегодня (для помощника) ──
  const isToday = (iso) => { const d = new Date(iso); return d.getDate() === now.getDate() && d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); };
  const todayExpenses = data.transactions.filter((t) => t.type === "expense" && isToday(t.date));
  const todaySpent = todayExpenses.reduce((s, t) => s + t.amount, 0);
  const todayByCat = (() => {
    const map = {};
    todayExpenses.forEach((t) => { map[t.category] = (map[t.category] || 0) + t.amount; });
    return Object.entries(map).map(([id, sum]) => ({ ...catById(id), sum })).sort((a, b) => b.sum - a.sum);
  })();
  ...
  // Выделено в этом месяце — сколько денег попало на конверт: переводы СЮДА + доход, зачисленный прямо на него
  // (например, зарплата Марины падает на её счёт как доход, а не переводом). Раньше учитывались только переводы,
  // из-за чего «потрачено X% выделенного» показывало абсурд вроде 1150%.
  const envAllocated = (eid) =>
    data.transfers.filter((t) => t.to === eid && inMonth(t.date)).reduce((s, t) => s + t.amount, 0) +
    monthTx.filter((t) => t.type === "income" && envOf(t) === eid).reduce((s, t) => s + t.amount, 0);
      ...
      <div className="max-w-md mx-auto pb-28">
        ...
        {tab === "overview" && (
          <div className="px-4 mt-4">
            ...
            {/* помощник — компактно: сегодняшние траты + подсказки одной карточкой */}
            {((isCurrentMonth && todayExpenses.length > 0) || tips.length > 0) && (
              <div className="bg-white rounded-3xl px-4 py-3 shadow-sm mb-4">
                {isCurrentMonth && (
                  <div className="flex items-center gap-2">
                    <span className="text-base">🤖</span>
                    <span className="font-black text-slate-800 text-sm flex-1">Сегодня</span>
                    <span className={`text-sm font-black ${todaySpent > 0 ? "text-rose-500" : "text-emerald-500"}`}>
                      {todaySpent > 0 ? `−${fmt(todaySpent)} zł` : "трат нет 👍"}
                    </span>
                  </div>
                )}
                {isCurrentMonth && todayByCat.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {todayByCat.map((c) => (
                      <span key={c.id} className="inline-flex items-center gap-1 rounded-full px-2 py-1 text-[11px] font-extrabold"
                        style={{ background: c.color + "18", color: c.color }}>
                        <span>{c.emoji}</span>{c.label} · {fmt(c.sum)}
                      </span>
                    ))}
                  </div>
                )}
                {tips.length > 0 && (
                  <div className={`space-y-1.5 ${isCurrentMonth ? "mt-2.5 pt-2.5 border-t border-slate-100" : ""}`}>
                    {tips.map((t, i) => (
                      <div key={i} className="flex items-start gap-2 text-[12px] font-bold leading-snug">
                        <span className={`mt-1 w-1.5 h-1.5 rounded-full shrink-0 ${
                          t.tone === "bad" ? "bg-rose-500" : t.tone === "warn" ? "bg-amber-500" : "bg-emerald-500"}`} />
                        <span className={
                          t.tone === "bad" ? "text-rose-600" : t.tone === "warn" ? "text-amber-700" : "text-emerald-600"}>
                          {t.text}
                        </span>
                      </div>
                    ))}
            ...
            {/* куда ушли деньги — свёрнуто (кольцо + список %) → развёрнуто (подробно) */}
            <div className="bg-white rounded-3xl p-5 shadow-sm mb-4">
              <button onClick={() => setSpendingOpen((o) => !o)} className="w-full flex items-center justify-between mb-3">
                <h2 className="font-black text-slate-800 text-lg">🍩 Куда ушли деньги</h2>
                {expenseCats.length > 0 && (
                  <span className={`text-slate-400 text-sm font-black transition-transform duration-300 ${spendingOpen ? "rotate-180" : ""}`}>▾</span>
                )}
              </button>
              {expenseCats.length === 0 ? (
                ...
                /* СВЁРНУТО */
                <button onClick={() => setSpendingOpen(true)} className="w-full flex items-center gap-4 text-left active:scale-[0.98] transition">
                  <Donut items={expenseCats} total={expense} centerLabel="zł ушло" size={128} />
                    ...
                    {expenseCats.slice(0, 5).map((c) => {
                      ...
                      return (
                        <div key={c.id} className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-black shrink-0" style={{ color: c.color }}>{pct}%</span>
                        </div>
                      );
                    })}
                    {expenseCats.length > 5 && (
                      <div className="text-[11px] font-extrabold text-slate-400 pl-[18px]">+ ещё {expenseCats.length - 5}</div>
                    )}
                  </div>
                </button>
              ) : (
                ...
                <>
                  ...
                  <div className="mt-4">
                    {expenseCats.map((c) => {
                      ...
                      const catTx = monthTx.filter((t) => t.type === "expense" && t.category === c.id)
                        .sort((a, b) => (a.date < b.date ? 1 : -1));
                      const count = catTx.length;
                      const isOpen = openCat === c.id;
                      return (
                        <div key={c.id} className="mb-3">
                          <button onClick={() => setOpenCat((o) => (o === c.id ? null : c.id))} className="w-full text-left active:opacity-70 transition">
                            ...
                            <div className="text-[11px] font-bold text-slate-400 mt-0.5 flex justify-between">
                              <span>{count} {count === 1 ? "операция" : count < 5 ? "операции" : "операций"} · в среднем {fmt(c.sum / count)} zł</span>
                              <span className={`shrink-0 transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`}>▾</span>
                            </div>
                          </button>
                          {isOpen && (
                            <div className="mt-2 mb-1 rounded-2xl bg-slate-50 p-2 space-y-0.5">
                              {catTx.map((t) => {
                                const d = new Date(t.date);
                                return (
                                  <div key={t.id} className="flex items-center gap-2 px-2 py-1.5">
                                    <span className="text-[11px] font-extrabold text-slate-400 shrink-0 w-14">{d.getDate()} {MONTHS_GEN[d.getMonth()].slice(0, 3)}</span>
                                    <span className="text-xs font-bold text-slate-600 flex-1 truncate">{t.note || c.label}</span>
                                    <span className="text-[11px] font-bold text-slate-400 shrink-0">{personById(t.personId).name}</span>
                                    <span className="text-sm font-black text-slate-800 shrink-0">{fmt(t.amount)} zł</span>
                                  </div>
                                );
                              })}
                            </div>
